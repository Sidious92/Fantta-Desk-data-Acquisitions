...
OUT = Path('.nexus-f2-understat-player-match-temporal-probe-v2-1')
RAW = OUT / 'raw'
...
UA = 'Mozilla/5.0 (compatible; FantaNexus-F2-Temporal-Probe/2.1)'
...
def fetch_wire(url, ajax=False):
    ...
    with urllib.request.urlopen(req,timeout=90) as r:
        ...
        body=r.read(); encoding=str(r.headers.get('Content-Encoding') or '').lower()
        return body, encoding

def decoded_body(wire, encoding):
    if wire.startswith(b'\x1f\x8b') or 'gzip' in encoding:
        return gzip.decompress(wire), 'gzip'
...
def season(v):
    ...
    if m:
        y=int(m.group(1))
        if int(m.group(2)) != (y+1)%100: raise RuntimeError(f'INVALID_SEASON:{s!r}')
        return s
...
def reconstruct(rows,pid,sea):
    ...
    for r in rows:
        ...
        if not re.fullmatch(r'\d{4}-\d{2}-\d{2}',d): raise RuntimeError(f'INVALID_DATE:{pid}:{sea}:{r.get("id")}:{r.get("date")!r}')
...
def main():
    ...
    csv_wire,csv_encoding=fetch_wire(CSV_URL); csv_raw,_=decoded_body(csv_wire,csv_encoding)
    ...
    for ix,subject in enumerate(COHORT):
        if ix: time.sleep(1)
        pid=subject['playerId']; wire,encoding=fetch_wire(API_URL.format(player_id=pid),ajax=True); raw,transport=decoded_body(wire,encoding)
        (RAW/f'player-{pid}-wire.bin').write_bytes(wire); (RAW/f'player-{pid}-api.json').write_bytes(raw)
        payload=json.loads(raw.decode('utf-8'))
        matches=payload.get('matches') if isinstance(payload,dict) else None
        if not isinstance(matches,list): raise RuntimeError(f'INVALID_API_SHAPE:{pid}:{type(payload).__name__}:{sorted(payload.keys()) if isinstance(payload,dict) else None}')
        ...
        evidence.append({**subject,'wireBytes':len(wire),'wireSha256':h(wire),'decodedBytes':len(raw),'decodedSha256':h(raw),'transportEncodingObserved':encoding,'transportDecodedAs':transport,'matchRows':len(matches),'pinnedSerieASeasonRows':len(rows_for),'payloadKeys':sorted(payload.keys())})
    ...
    report={'schema':'NEXUS_F2_UNDERSTAT_PLAYER_MATCH_TEMPORAL_PROBE_V2_1','status':status,'capturedAt':datetime.now(timezone.utc).isoformat(),'source':{'provider':'UNDERSTAT','endpoint':'getPlayerData/{player_id}','endpointMethod':'GET_X_REQUESTED_WITH_XMLHTTPREQUEST','pinnedAggregateCommit':UNDERSTAT_COMMIT,'pinnedAggregateSha256':UNDERSTAT_SHA256},'supersedesFailedProbe':'NEXUS_F2_UNDERSTAT_PLAYER_MATCH_TEMPORAL_PROBE_V2','correction':'Decode gzip wire response before parsing current AJAX JSON endpoint.','cohort':{'players':len(COHORT),'selection':'DETERMINISTIC_EXACT_F1_D1_BOUND_LONG_HISTORY_TWO_PER_CLASSIC_ROLE'},'matchShape':{'totalRows':total,'datedRows':dated,'allRowsDated':total>0 and total==dated},'reconciliation':{'comparisons':len(comps),'exactComparisons':exact,'allComparedSeasonsExact':len(comps)>0 and exact==len(comps),'fields':OBSERVED,'rows':comps},'temporalDecision':{'observedEventStableFields':'ELIGIBLE_FOR_FULL_RECONSTRUCTION_PILOT_IF_PROBE_PASS','expectedModelFields':EXPECTED,'expectedModelFieldsReleased':False,'expectedMetricReason':'Match occurrence dates do not establish provider-model expected-metric historical vintage/invariance.'},'playerEvidence':evidence,'failures':failures,'governance':{'newProviderIntroduced':False,'fuzzyMatchingUsed':False,'f2ParametersFitted':False,'expectedMetricsPromoted':False,'canonicalPredictiveEngineModified':False}}
    ...
    mm=write_json(OUT/'MANIFEST.json',{'schema':'NEXUS_F2_UNDERSTAT_PLAYER_MATCH_TEMPORAL_PROBE_MANIFEST_V2_1','status':status,'probe':pm,'rawFiles':raw_meta})
