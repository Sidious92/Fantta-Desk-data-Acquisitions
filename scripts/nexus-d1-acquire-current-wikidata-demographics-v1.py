#!/usr/bin/env python3
"""Acquire D1 current-505 identity/DOB evidence from Wikidata.
...
UA='FantaNexus-D1-Current505/1.0 (+https://github.com/Sidious92/Fantta-Desk-data-Acquisitions)'
...
def short_name_matches(provider_name, entity_names):
    ...
    for n in entity_names:
        ...
        if not all(any(bt==t or bt==joined for t in toks) for bt in base):
            # allow apostrophe/hyphen collapsed surname, e.g. N'Dicka -> ndicka
...
def main():
    ...
    # primary search only; full historical name is first whenever bridge exists
    for idx,s in enumerate(subjects):
        ...
        try:
            ...
            p=search_dir/f'{idx:03d}--{slug(s["subjectId"])}--{slug(lookup)}.json'; p.write_bytes(b)
            hits=[h.get('id') for h in payload.get('search') or [] if h.get('id')]
            searches[s['subjectId']]={'lookup':lookup,'hits':hits,'rawPath':str(p.relative_to(root)),'rawSha256':sha(b)}; qids.update(hits)
    ...
    for s in subjects:
        ...
        if s.get('bridgePersonKey'):
            ...
            # unbridged short-name identity must have exact unique club context
            if len(context_matches)==1: chosen,_=context_matches[0]; method='PROVIDER_SHORT_PATTERN_PLUS_UNIQUE_CONTEXT_CLUB'

        base={'subjectId':s['subjectId'],'fantacalcioSourcePlayerId':s['fantacalcioSourcePlayerId'],'lookupName':lookup,'contextClubs':s['contextClubs'],'classicRole':s['classicRole'],'bridgePersonKey':s.get('bridgePersonKey'),'identityFoundationState':s.get('identityFoundationState'),'candidateCount':len(candidates),'contextEvidence':context,'searchEvidence':se,'historicalAdmissibility':'NOT_ESTABLISHED_CURRENT_ACQUISITION'}
        if chosen:
    ...
    result={'schema':'NEXUS_D1_CURRENT_2026_27_WIKIDATA_DEMOGRAPHICS_RESULT_V1','protocolVersion':'1.1','status':'PASS' if not failures else 'REVIEW_REQUIRED','declaredUse':'CURRENT_PROBE_POPULATION_COVERAGE','capturedAt':now(),'subjectSurface':{'count':505,'canonicalAsOf':'2026-08-18','decodedJsonSha256':manifest['payload']['decodedJsonSha256']},'rules':{'fuzzyMatchingUsed':False,'dobUsedForIdentitySelection':False,'unbridgedRequiresUniqueExactClubContext':True,'computedAgeDerived':False,'currentRetrievalImpliesHistoricalAsOf':False,'trainingPromotionGranted':False,'requestFailureCanBecomeMissingIdentity':False},'summary':{'subjects':505,'mappingStatus':mc,'dobStatus':dc,'bridgedMappingStatus':bridge_mc,'unbridgedMappingStatus':unbridged_mc,'requestFailures':len(failures)},'requestFailures':failures,'records':records}
    ...
    (root/'MANIFEST.json').write_text(json.dumps({'schema':'NEXUS_D1_CURRENT_2026_27_WIKIDATA_MANIFEST_V1','generatedAt':now(),'files':files,'fileCount':len(files),'canonicalContentSha256':digest},indent=2)+'\n')
