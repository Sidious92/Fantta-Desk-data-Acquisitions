...
# Copy exact historical raw CSVs from pinned public archive.
...
for year in YEARS:
 ...
 files=sorted(dst.glob('rodada-*.csv'))
 rows=0;ids=set();cols=set();rounds=[]
 for p in files:
  ...
  historical_inventory.append({'year':year,'file':str(p.relative_to(OUT)),'bytes':p.stat().st_size,'sha256':sha_file(p),'rows':n})
 year_stats[str(year)]={'round_files':len(files),'rows':rows,'unique_player_ids':len(ids),'round_min':min(rounds) if rounds else None,'round_max':max(rounds) if rounds else None,'columns':sorted(cols)}
 total_rows+=rows;total_files+=len(files)

# Current provider API snapshot; no authenticated routes.
api_paths={
 'market_status':'/mercado/status',
 'rounds':'/rodadas',
 'clubs':'/clubes',
 'matches':'/partidas',
 'market_players':'/atletas/mercado',
 'scored_players':'/atletas/pontuados'
}
api_meta={};api_objs={}
for name,path in api_paths.items():
 try:
  ...
  api_meta[name]={'url':API+path,'error':repr(e)}

market=api_objs.get('market_players') or {}
athletes=market.get('atletas') if isinstance(market,dict) else None
...
manifest={
 'schema':'NEXUS_CARTOLA_FC_BRASILEIRAO_V1',
 'status':'PASS',
 'provider':'Cartola FC',
 'competition':'Campeonato Brasileiro Serie A',
 'archive':{'repository':'henriquepgomide/caRtola','pinned_commit':PIN,'observed_head':head,'years':YEARS,'year_stats':year_stats,'total_round_files':total_files,'total_rows':total_rows,'column_inventory':sorted(all_columns)},
 'current_api':{'base':API,'surfaces':api_meta,'market_players':len(athletes),'market_player_fields':market_fields,'market_scout_fields':scout_fields},
 'governance':{'authenticated_surface_accessed':False,'cross_provider_normalization_performed':False,'missing_values_filled':False,'predictive_models_modified':False,'decision_layer_started':False,'provider_native_fields_preserved':True,'historical_seasons_merged_at_ingest':False}
}
checks={
 'archive_pin':head==PIN,
 'all_years':all((HIST/str(y)).is_dir() for y in YEARS),
 'round_files_gt_100':total_files>100,
 'historical_rows_gt_10000':total_rows>10000,
 'current_market_players_gt_100':len(athletes)>100,
 'no_auth':manifest['governance']['authenticated_surface_accessed'] is False,
 'no_model_mutation':manifest['governance']['predictive_models_modified'] is False
}
manifest['validation']={'status':'PASS' if all(checks.values()) else 'FAIL','checks':checks}
...
print(json.dumps({'status':manifest['status'],'archive_files':total_files,'archive_rows':total_rows,'market_players':len(athletes),'year_stats':{y:{k:v for k,v in s.items() if k!='columns'} for y,s in year_stats.items()}},ensure_ascii=False,indent=2))
