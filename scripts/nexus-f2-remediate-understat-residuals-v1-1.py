...
UA='Mozilla/5.0 (compatible; FantaNexus-F2-Residual-Remediation/1.1)'
ALLOWED_DUPLICATE_DIFFS={'roster_id','xG','npxG','xA','xGChain','xGBuildup'}
EVENT_IDENTITY_FIELDS=['id','season','date','h_team','a_team']
RELEASED_DUPLICATE_FIELDS=['time','goals','npg','assists','shots','key_passes']
...
ROSTER_SOURCE_RUN_ID='32665685119'
ROSTER_SOURCE_PACKAGE_SHA256='406292ed47fc89dbb39b94ce1c161bf31c911e90e683629e86db0705657e6b0b'
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--input-root',required=True); ap.add_argument('--roster-root',required=True); ap.add_argument('--output-root',required=True); args=ap.parse_args()
    root=Path(args.input_root); roster_source=Path(args.roster_root); out=Path(args.output_root); out.mkdir(parents=True,exist_ok=True); roster_raw=out/'match-roster-raw'; roster_raw.mkdir(exist_ok=True)
    failures=[]
    ...
    for pid in sorted(pid_person,key=lambda x:(int(x) if x.isdigit() else 10**18,x)):
        ...
        for mid,rows in by_id.items():
            ...
            differing=[k for k in all_keys if len({canon(r.get(k)) for r in rows})>1]
            identity_equal=all(len({canon(r.get(k)) for r in rows})==1 for k in EVENT_IDENTITY_FIELDS)
            observed_equal=all(len({canon(r.get(k)) for r in rows})==1 for k in RELEASED_DUPLICATE_FIELDS)
            allowed_diffs=set(differing).issubset(ALLOWED_DUPLICATE_DIFFS)
            if identity_equal and observed_equal and allowed_diffs:
                dedup.append(rows[0]); duplicate_audit.append({'playerId':pid,'matchId':mid,'duplicateRows':len(rows),'status':'OBSERVED_EVENT_DUPLICATE_COLLAPSED_PER_F2B_AMENDMENT_V1','differingFields':differing,'releasedObservedFieldsEqual':True,'rawRowsPreservedInUpstreamArtifact':True})
            else:
                failures.append({'code':'DUPLICATE_EVENT_CONFLICT_AFTER_AMENDMENT','playerId':pid,'matchId':mid,'rows':len(rows),'differingFields':differing,'identityEqual':identity_equal,'releasedObservedFieldsEqual':observed_equal,'allowedDifferenceSet':allowed_diffs})
    ...
    for mid in sorted({k[0] for k in ambiguous_requests},key=lambda x:(int(x) if x.isdigit() else 10**18,x)):
        src=roster_source/f'match-{mid}-api.json'; wire_src=roster_source/f'match-{mid}-wire.bin'
        if not src.exists() or not wire_src.exists():
            failures.append({'code':'FROZEN_MATCH_ROSTER_EVIDENCE_MISSING','matchId':mid}); continue
        raw=src.read_bytes(); wire=wire_src.read_bytes()
        try: payload=json.loads(raw.decode('utf-8'))
        except Exception as exc: failures.append({'code':'FROZEN_MATCH_ROSTER_JSON_INVALID','matchId':mid,'error':repr(exc)}); continue
        ...
        roster_meta.append({'matchId':mid,'wireBytes':len(wire),'wireSha256':H(wire),'decodedBytes':len(raw),'decodedSha256':H(raw),'sourceRunId':ROSTER_SOURCE_RUN_ID})
    ...
    report={'schema':'NEXUS_F2B_UNDERSTAT_RESIDUAL_REMEDIATION_V1_1','status':status,'capturedAt':datetime.now(timezone.utc).isoformat(),'source':{'runId':SOURCE_RUN_ID,'artifactDigestSha256':SOURCE_ARTIFACT_SHA256,'packageSha256':SOURCE_PACKAGE_SHA256,'rosterEvidenceRunId':ROSTER_SOURCE_RUN_ID,'rosterEvidencePackageSha256':ROSTER_SOURCE_PACKAGE_SHA256,'pinnedAggregateSha256':US_SHA256},'counts':{'players':len(pid_person),'sourceReferences':len(reconciliation),'preRemediationAmbiguousPlayerMatchPairs':len(ambiguous_requests),'uniqueRosterMatchRequests':len({x[0] for x in ambiguous_requests}),'rosterResolutions':len(resolution_audit),'observedEventDuplicateRowsCollapsed':sum(x['duplicateRows']-1 for x in duplicate_audit),'postRemediationMismatches':len(bad),'postRemediationUnresolvedAssignments':len(unresolved_assign),'segments':len(segments)},'files':metas,'failures':failures,'governance':{'fuzzyMatchingUsed':False,'currentGroupsUsedToOverridePinnedAggregate':False,'expectedMetricsReleased':False,'canonicalPredictiveEngineModified':False,'f2ParametersFitted':False},'temporalDecision':{'observedFieldsStatus':'VERIFIED_EVENT_RECONSTRUCTION' if status=='PASS' else 'BLOCKED','expectedFieldsStatus':'QUARANTINED_VINTAGE_REQUIRED'}}
    ...
    write_json(out/'MANIFEST.json',{'schema':'NEXUS_F2B_UNDERSTAT_RESIDUAL_REMEDIATION_MANIFEST_V1_1','status':status,'audit':rep,'files':man,'treeDigestSha256':H('\n'.join(f"{x['path']}\t{x['sha256']}\t{x['bytes']}" for x in man).encode())})
    print(json.dumps({'status':status,'players':len(pid_person),'references':len(reconciliation),'ambiguousPairs':len(ambiguous_requests),'resolutions':len(resolution_audit),'collapsedObservedDuplicateRows':sum(x['duplicateRows']-1 for x in duplicate_audit),'mismatches':len(bad),'unresolvedAssignments':len(unresolved_assign),'failures':len(failures)},indent=2))
