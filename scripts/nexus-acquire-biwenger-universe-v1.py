...
OUT=Path(os.environ.get('NEXUS_BIWENGER_UNIVERSE_OUT','/mnt/data/nexus-biwenger-universe-v1'))
...
def req(url,params=None,attempts=4):
    ...
    for i in range(attempts):
        try:
            r=requests.get(url,params=params or {},headers=HEADERS,timeout=35); b=r.content
            try:o=r.json()
...
def main():
    ...
    for slug in CANDIDATES:
        ...
        for score in SCORES:
            ...
            try:
                ...
                valid=len(players)>0
                rec={'candidate_slug':slug,'score_query':score,'status':s,'url':u,'bytes':len(b),'sha256':sha(b),'valid':valid,'players':len(players),'data_slug':data.get('slug'),'competition_id':data.get('id'),'competition_name':data.get('name'),'scoreID':data.get('scoreID'),'season':data.get('season')}
                if valid:
                    valid_competitions.add(slug);modes.append(score)
                    d=mkdir(raw/'catalogs'/slug); fname=f"score-{'none' if score is None else score}.json"; path=d/fname; path.write_bytes(b)
                    for p in players:
                        ...
                        catalog_rows.append({'provider':'BIWENGER','competition_candidate_slug':slug,'competition_name':data.get('name'),'competition_id':data.get('id'),'season_json':json.dumps(data.get('season'),ensure_ascii=False,default=str,separators=(',',':')),'score_query':score,'provider_scoreID':data.get('scoreID'),'provider_player_id':pid,'player_name':p.get('name'),'player_slug':p.get('slug'),'team_id':p.get('teamID'),'points':p.get('points'),'points_last_season':p.get('pointsLastSeason'),'source_url':u,'source_sha256':sha(b),'source_local_path':str(path.relative_to(OUT)),'provider_fields_json':json.dumps(p,ensure_ascii=False,default=str,separators=(',',':'))})
                catalog_manifest.append(rec)
            except Exception as exc:
                catalog_manifest.append({'candidate_slug':slug,'score_query':score,'valid':False,'error':str(exc)})
        valid_modes[slug]=modes
    dump(OUT/'catalog-manifest.json',catalog_manifest); write_csv(norm/'catalog-player-score-rows.csv',catalog_rows)
    ...
    def one(item):
        ...
        params={'lang':'en','fields':'*,team,seasons,competition'}
        modes=valid_modes.get(slug) or []
        if modes and modes[0] is not None:params['score']=modes[0]
        url=f'{BASE}/players/{slug}/{pslug}'
        try:
            o,b,u,s=req(url,params); d=(o.get('data') or {}) if isinstance(o,dict) else {}
            path=mkdir(meta_root/slug)/f'{pid}-{pslug}.json';path.write_bytes(b)
            return {'item':item,'data':d,'url':u,'status':s,'sha256':sha(b),'path':path}
        except Exception as exc:return {'item':item,'url':url,'error':str(exc)}
    with ThreadPoolExecutor(max_workers=6) as ex:
        futs=[ex.submit(one,x) for x in identities.values()]
        for fut in as_completed(futs):
            r=fut.result(); it=r['item']
            if r.get('error'):
                metadata_errors.append({**it,'url':r.get('url'),'error':r['error']});continue
    ...
    manifest={'schema':'NEXUS_BIWENGER_UNIVERSE_V1','status':'PASS' if len(valid_competitions)>=5 and rate>=0.95 else 'FAIL_VALIDATION','capture_started':started,'capture_completed':now(),'coverage':{'candidate_competitions':len(CANDIDATES),'valid_competitions':sorted(valid_competitions),'valid_competition_count':len(valid_competitions),'catalog_score_rows':len(catalog_rows),'unique_competition_player_identities':len(identities),'metadata_ok':len(metadata),'metadata_errors':len(metadata_errors),'metadata_success_rate':rate,'season_references':len(season_refs)},'valid_score_queries_by_competition':valid_modes,'governance':{'cross_score_mode_normalization_performed':False,'cross_competition_normalization_performed':False,'missing_values_filled':False,'predictive_models_modified':False,'decision_layer_started':False,'history_reports_downloaded':False}}
    ...
    dump(OUT/'file-inventory.json',inv);print(json.dumps({'status':manifest['status'],'coverage':manifest['coverage'],'valid_scores':valid_modes,'files':len(inv)},ensure_ascii=False,indent=2))
