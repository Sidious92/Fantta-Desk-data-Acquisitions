...
OUT=Path('data/nexus-d1/final-v1')

def now(): return datetime.now(timezone.utc).isoformat().replace('+00:00','Z')
def sha(b:bytes): return hashlib.sha256(b).hexdigest()
def load(p):
    b=Path(p).read_bytes(); return json.loads(b), {'path':str(p),'size':len(b),'sha256':sha(b)}
def key(kind, value): return f"nexus-{kind}-v1-{sha((kind+'|'+str(value)).encode())[:24]}"
def sid(r):
    s=r.get('subjectId')
...
def fc_current_id(r):
    for k in ('fantacalcioPlayerId','fantacalcioId','playerId','sourcePlayerId','sourceIdentityId'):
        v=r.get(k)
...
# Second-pass metadata retained from the first-pass records without performance fields.
...
# Historical person bridge is the D0-derived identity authority for all 2048.
hist_person_key={}
...
# Final historical DOB overrides.
hist_override={}
for r in hopen['records']:
    if r.get('dateOfBirth'): hist_override[str(r['subjectId'])]={'dateOfBirth':r['dateOfBirth'],'status':'DOB_VERIFIED','method':'EXACT_FC_OBSERVATION_WITH_D0_BRIDGE'}
for r in resid['historicalOpenResiduals']:
    if r.get('dateOfBirth'): hist_override[str(r['subjectId'])]={'dateOfBirth':r['dateOfBirth'],'status':'DOB_VERIFIED','method':'EXACT_FC_CONSENSUS_WITH_D0_BRIDGE'}
for r in hconf['records']:
    if r.get('dateOfBirth'): hist_override[str(r['subjectId'])]={'dateOfBirth':r['dateOfBirth'],'status':'DOB_VERIFIED','method':'DOB_CONFLICT_RESOLVED_BY_EXACT_FC_OBSERVATION'}
for r in resid['historicalDobConflictResiduals']:
    if r.get('dateOfBirth'):
        hist_override[str(r['subjectId'])]={'dateOfBirth':r['dateOfBirth'],'status':'DOB_VERIFIED','method':'DOB_CONFLICT_RESOLVED_BY_EXACT_FC_CONSENSUS'}
    else:
...
def add_person(pk, scope, subject_id, dobrec, aliases, observed_name, identity_method, source_status):
    ...
    ref={'scope':scope,'subjectId':subject_id,'identityMethod':identity_method,'sourceIdentityStatus':source_status}
    p['subjectRefs'].append(ref);p['providerAliases'].extend(aliases)
...
for r in hr:
    ...
    if s in hist_override: d=hist_override[s]
        ...
        d={'dateOfBirth':r['dateOfBirth'],'status':'DOB_VERIFIED','method':'WIKIDATA_FIRST_PASS'}
...
# Current 505, using second-pass metadata when relevant.
cur_verified=0
for r in cr:
    ...
    if s in cur_override:
        rr=cur_override[s]; d={'dateOfBirth':rr['dateOfBirth'],'status':'DOB_VERIFIED','method':'CURRENT_LISTONE_EXACT_FANTACALCIO_ID'}; nm=rr.get('fullName') or name_of(merged)
    elif r.get('dateOfBirthStatus')=='DOB_VERIFIED' and r.get('dateOfBirth'):
        d={'dateOfBirth':r['dateOfBirth'],'status':'DOB_VERIFIED','method':'WIKIDATA_FIRST_PASS'}; nm=name_of(merged)
...
# Consolidate person-level DOB without silently resolving conflicts.
for p in persons.values():
    ...
    verified={x['dateOfBirth'] for x in p['demographicsEvidence'] if x['status']=='DOB_VERIFIED' and x.get('dateOfBirth')}
...
# 445 never-Understat-bridged groups remain provider-scoped identity units, never global persons.
units=[]
for g in g445['groups']:
    idx=g['sourceSubjectIndex'];fc=str(g['fantacalcioPlayerId']);sigs=g.get('signatures') or []
    dobs=sorted({x.get('dateOfBirth') for x in sigs if x.get('dateOfBirth')})
...
manifest={'schema':'NEXUS_D1_FINAL_PACKAGE_MANIFEST_V1','protocolVersion':'1.1','status':'PASS','generatedAt':captured,'outputs':{'identityMaster':{'path':'IDENTITY_MASTER.json','size':len(mb),'sha256':sha(mb)},'registry':{'path':'D1_REGISTRY.json','size':len(rb),'sha256':sha(rb)}},'sourceAuthorities':sources,'governance':master['rules']}
