...
def main():
    ...
    # Season player totals. Provider uses opaque string ids.
    ...
    for _ in range(50):
        ...
        for p in arr:
            ...
            field_set.update(p.keys());pid=p.get('id')
            if pid is None:continue
            key=str(pid)
            if key not in players:new+=1;players[key]=p
            occurrences.append({'provider':'FANTASY_PRO_LEAGUE','competition':'Jupiler Pro League','season_id':SEASON,'provider_player_id':key,'source_url':r.url,'source_sha256':sha(b),'provider_fields_json':json.dumps(p,ensure_ascii=False,default=str,separators=(',',':')),**scalar(p)})
    ...
    def one(pid):
        try:
            r=get(f'{API}/player/{pid}',{'withAggregatedWeekStats':1,'withStats':1});b=r.content;o=r.json();p=details/f'{pid}.json';p.write_bytes(b);return {'id':pid,'obj':o,'url':r.url,'sha256':sha(b),'path':p}
        except Exception as exc:return {'id':pid,'error':str(exc)}
    with ThreadPoolExecutor(max_workers=6) as ex:
        futs=[ex.submit(one,pid) for pid in sorted(players)]
        for fut in as_completed(futs):
            x=fut.result();pid=x['id']
            if x.get('error'):detail_errors.append({'provider_player_id':pid,'error':x['error']});continue
            o=x['obj'];
            if isinstance(o,dict):
                detail_top_fields.update(o.keys())
                # Inventory nested stats keys without assuming their scoring meaning.
                def walk(v):
    ...
    # Public read endpoints referenced by frontend; failures remain diagnostic.
    public_reads=[]
    for path,params in [('points-confirmation',{'competitionFeed':FEED,'seasonId':SEASON}),('matches/info',{'competitionFeed':FEED,'seasonId':SEASON}),('clubs',{'competitionFeed':FEED,'seasonId':SEASON})]:
        try:
            r=get(f'{API}/{path}');b=r.content;p=raw/(path.replace('/','-')+'.json');p.write_bytes(b);public_reads.append({'endpoint':path,'status':r.status_code,'url':r.url,'bytes':len(b),'sha256':sha(b),'local_path':str(p.relative_to(OUT))})
